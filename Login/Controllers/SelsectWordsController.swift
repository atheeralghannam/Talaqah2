...
class SelsectWordsController: UIViewController {
    ...
    @IBAction func wordButtonPressed(_ sender: UIButton) {
        if sender.currentTitle! == "الصفات"{
            if !isthere[1]{
                SCLAlertView().showCustom( "نأسف", subTitle: "حسب إعداداتك لا يوجد تمرين مناسب هنا", color: UIColor(named: "Silver")! , icon: UIImage(named: "excmark")!, closeButtonTitle: "حسنًا")
            }
            else{
                adj = true
